 ...
 execute(){
   ...
   return new Promise(async(resolve,reject)=>{
     try{
      ...
      var node = tableContents[0][TABLE_COL[0]];
      // Hold ctrl and then add the node.
      await this.page.keyboard.down('ctrl').catch((reason)=>{reject(reason)});
      // Get the graph and node position, then sum them up and click
      await this.clickOnNodeInGraph(node).catch((reason)=>{reject(reason)});
      await this.page.keyboard.up('ctrl').catch((reason)=>{reject(reason)});
      ...
      numberOfDGRs = await this.page.evaluate(EVALUATE_SETS).catch((reason)=>{reject(reason)});
      if(numberOfDGRs !== 1)

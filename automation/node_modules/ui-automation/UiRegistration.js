...
let UiRegistration = (superclass) => class extends superclass {
	...
	unregister( login ) {
		return new Promise( async ( resolve, reject ) => {
			...
			try {
				var checkForUser = ( message ) => { return ( hasUser ) => { if( hasUser ) { reject( message ); } else { resolve(); }}};

				await this.userdb( select, checkForUser( `Failed to unregister: User ${login} not registered` ));
				await this.userdb( unregister );
				await this.userdb( select, checkForUser( `Failed to unregister: User ${login} still registered.`));

	...
	execute() {
	    return new Promise( async ( resolve, reject ) => {
			try {
				...
				// ===== GET THE RESULTS FROM THE TABLE AND FLATTEN THE NESTED (GROUPED) ARRAY TO 1D ARRAY
				let grouped   = await this.table.results();
				let synonyms  = this.table.getSynonyms( dgr, grouped ); // MW WORKAROUND; should directly query the DB for aliases, but this doesn't seem to work
				let regex     = new RegExp( synonyms.join( '|' ));
				let flattened = [];
				grouped.forEach(( group ) => { flattened.push( group ); flattened = flattened.concat( group.details ); });

...
class GroupByTest extends mix( Test ).with( Table, GroupBy ) {
    ...
    execute() {
        ...
        return new Promise( async ( resolve, reject ) => {
			Array.prototype.equals = ( other ) => { return this.every( x => other.includes( x )) && other.every( x => this.includes( x )); }
			...
			let test = { groupBy: {
				dgrPair: async () => {
					...
					let pass  = table.every(( group ) => { let groupPair = [ group.DGR1, group.DGR2 ]; return group.details.every(( row ) => { let rowPair = [ row.DGR1, row.DGR2 ]; return rowPair.equals( groupPair ); }); });

...
class Onehop extends Test{
  ...
  return new Promise(async(resolve,reject)=>{
		try{
			...
			await this.page.click('.download',{waitUntil : 'networkidle2'}).catch((reason)=>{reject(reason);})
			await this.page.click('.ajs-ok',{waitUntil : 'networkidle2'}).catch((reason)=>{reject(reason);})
			...
			//test passed
			resolve(this.createResponse(true,`Tested Save and Upload  successfully`,this.priority));
		}catch(e){
		  console.log(e);
			//test failed

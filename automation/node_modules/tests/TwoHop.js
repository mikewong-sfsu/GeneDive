	...
	execute() {
	    return new Promise( async ( resolve, reject ) => {
			try {
				...
				let synonyms  = dgrs.reduce(( synonyms, dgr ) => { return synonyms.concat( this.table.getSynonyms( dgr, grouped )); }, []);
				let regex     = new RegExp( synonyms.join( '|' ));
				let flattened = grouped.reduce(( flattened, group ) => { flattened.push( group ); return flattened.concat( group.details ); }, []);

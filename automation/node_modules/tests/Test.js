...
class Test {
	...
	*/
	...
	async gotoSearchPage() { return this.navigate( `${this.options.url}/search.php` ); }
	async gotoIndexPage()  { return this.navigate( `${this.options.url}/index.php` ); }
	...
	pageIsSearchPage() { return this.pageMatch( /\bsearch.php$/ ); }
	pageIsIndexPage()  { return this.pageMatch( /\bindex.php$/ ); }
	...
	// ============================================================
		return new Promise( async( resolve, reject ) => {
			try {
				...
				if( ! this.pageIsSearchPage()) { reject( 'Login not redirected to search page' ); }
				resolve();
	...
	// ============================================================
		return new Promise( async ( resolve, reject ) => {
			try {
				...
				if( ! this.pageIsIndexPage()) { reject( 'Logout not redirected to landing page' ); }
				resolve();
	...
	searchMode( mode ) {
		return new Promise( async ( resolve, reject ) => { 
			try { 
				if( ! this.pageIsSearchPage()) { reject( 'Search not directed to search page' ); }
				await this.pageToLoad(); // Allow cache results to load via AJAX call
				await this.page.waitForSelector( '.table-help' ); // Search is ready when the help page or results table are displayed
	...
	// ============================================================
		...
		return new Promise( async ( resolve, reject ) => {
			...
			try {
				...
				await this.page.waitForSelector( '.table' ); // Search is complete when table of results is rendered

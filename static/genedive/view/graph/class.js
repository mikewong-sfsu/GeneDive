class GraphView {
    ...
    initializeListeners() {
        $(document).on('interactions-updated', function() {
            // Mike's suggestion below: try timeOut() function as shown below to resolve Graph View tab delay issue - 
            //     specifically, on the second or subsequent filtering activity, Table View must be viewed before Graph View is visible.
            // Mike suggested using setTimeout() in every listener relating to the views.
            //var timeout = setTimeout(function() { Webdiggr.GraphView.displayGraph(Webdiggr.FilterControls.filtered_interactions);}, 0);
            Webdiggr.GraphView.displayGraph(Webdiggr.FilterControls.filtered_interactions);
        });
    }

    displayGraph(interactions) {

        // Commenting out the below lines means that the graph recalculates every time, as instructed during Feb 28 meeting/
        // execute updateGraph() if an original, unfiltered graph has already been displayed.
        // Filtering actions will only update an existing graph, never add nodes to it.
        /*
        if (!this.isOriginalGraph) {
            console.log("entering filtered interactions");
            this.filteredInteractions = interactions;
            Webdiggr.GraphView.updateGraph(interactions);
            return;
        }
        */
        ...
        var svg = d3.select("#graph-display")
            ...
            // make the svg object pannable/zoomable
            /*
